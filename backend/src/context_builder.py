...
def format_passages(passages: list[Passage], label_prefix: str = "S") -> str:
    ...
    so a fact about one subject cannot silently cite the other's evidence.
    """
    if not passages:
...
def labels_for(passages: list[Passage], label_prefix: str = "S") -> list[str]:
    ...
    The citation checker needs the exact set of labels that exist, not just how
    many, because a comparison produces two sets ([A1..] and [B1..]) and a
    label from one side must not validate against the other.
    """
