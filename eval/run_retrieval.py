...
def evaluate(tag: str, provider: str = "huggingface", top_k: int = TOP_K) -> dict:
    """
    provider / top_k exist so that "should we change the embedding provider or
    top_k?" is answered by this harness, not by opinion. Run once per variant
    with a distinct tag, then --compare.

    The provider is named explicitly rather than falling back, because a
    measurement that silently changed embedding space halfway would be
    meaningless.
    """
    gold = load_gold()
    model = create_provider(provider)
    ...
    for item in gold:
        ...
        ctx = build_context(entries, idx.store, debug=False,
                            hard_floor=model.hard_floor, soft_floor=model.soft_floor)
...
def main() -> None:
    ...
    ap.add_argument("--provider", default="huggingface",
                    help="embedding provider to measure: huggingface | gemini")
    ...
    if args.compare:
        ...
        return
    print_report(evaluate(args.tag, provider=args.provider, top_k=args.top_k))
